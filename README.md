...
| `multinode`                            | When true, use LeaderWorkerSet for both decode and prefill instead of the configured `workloadKind`                    | bool            | `false`                                     |
| `workloadKind`                         | Global workload kind for decode and prefill pods. One of `Deployment`, `StatefulSet`, `LeaderWorkerSet`. Overridden by `multinode`, `requester.enable` (decode only), and per-role `workloadKind`. | string          | `Deployment`                                |
...
| `decode.create`                        | If true, creates the decode workload (Deployment, StatefulSet, LeaderWorkerSet, or ReplicaSet depending on `workloadKind`/`multinode`/`requester.enable`) | bool            | `true`                                      |
| `decode.workloadKind`                   | Workload kind for decode pods. Overrides global `workloadKind`. One of `Deployment`, `StatefulSet`, `LeaderWorkerSet` | string          | `unset`                                     |
| `decode.annotations`                   | Annotations added to the decode workload                                                                          | Dict            | {}                                          |
| `decode.tolerations`                   | Tolerations added to the decode pod spec                                                                          | List            | []                                          |
| `decode.replicas`                      | Number of replicas for decode pods                                                                                | int             | 1                                           |
| `decode.volumeClaimTemplates`          | StatefulSet `volumeClaimTemplates` for per-pod persistent storage. Only applies when `workloadKind` is `StatefulSet`. A template named `model-storage` takes ownership of the model volume. | List            | []                                          |
| `decode.updateStrategy`                | StatefulSet `updateStrategy`. Only applies when `workloadKind` is `StatefulSet`. Use `decode.strategy` for Deployments. | object          | `{}`                                        |
| `decode.podManagementPolicy`           | StatefulSet `podManagementPolicy` (`Parallel` or `OrderedReady`). Only applies when `workloadKind` is `StatefulSet`. | string          | `Parallel`                                  |
| `decode.revisionHistoryLimit`          | StatefulSet `revisionHistoryLimit`. Only applies when `workloadKind` is `StatefulSet`.                            | int             | `unset`                                     |
| `decode.minReadySeconds`               | StatefulSet `minReadySeconds`. Only applies when `workloadKind` is `StatefulSet`.                                 | int             | `unset`                                     |
